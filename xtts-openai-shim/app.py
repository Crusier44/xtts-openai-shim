...
async def speech(req: SpeechRequest):
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        # Auto-switch models on demand: only pay the reload cost when the
        # requested voice actually needs a different model than whatever
        # is currently loaded. Consecutive messages using the same
        # character are effectively free after the first switch.
        ...
        if r.status_code != 200:
            # Surface XTTS's own error message rather than a generic 500,
            # since it's almost always something actionable (bad path,
