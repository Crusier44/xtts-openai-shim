...
async def on_startup():
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        # Ask XTTS what it actually has loaded right now, rather than
        # assuming nothing is loaded. This matters because the shim can
        # restart (e.g. redeploying a new image) independently of
        # xtts-api-server, which keeps running with whatever model it had
        # -- without this check, the shim's first request after a restart
        # would always try an unnecessary (and rejected) switch.
        try:
            r = await client.get(f"{XTTS_BASE_URL}/get_tts_settings")
            if r.status_code == 200:
                settings = r.json()
                # Field name varies by xtts-api-server version -- try the
                ...
                if loaded:
                    ...
                    log.info("Discovered already-loaded model on startup: %s", loaded)
        except Exception as e:
            log.warning("Could not query current model on startup: %s", e)
    ...
    async with _xtts_lock:
        async with httpx.AsyncClient(timeout=120) as client:
            ...
            if wanted_model is not None and wanted_model != _current_model:
                ...
                if r.status_code != 200:
                    # xtts-api-server rejects a switch to whatever model
                    # it already has loaded (e.g. after the shim itself
                    # restarted and lost track of _current_model while
                    # xtts-api-server kept running). That's not a real
                    # failure -- it confirms the model we wanted is
                    # already active, so just adopt it and move on.
                    already_loaded = "already loaded" in r.text.lower()
                    if already_loaded:
                        log.info(
                            "XTTS reports '%s' already loaded (shim's tracking was stale) -- continuing",
                            wanted_model,
                        )
                    else:
                        raise HTTPException(
...
async def admin_switch_model(model_name: str):
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        if r.status_code != 200:
            if "already loaded" in r.text.lower():
                _current_model = model_name
                return {"message": f"'{model_name}' was already loaded", "already_loaded": True}
            raise HTTPException(status_code=r.status_code, detail=r.text)
        _current_model = model_name
    return r.json()
